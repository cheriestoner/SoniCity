// View switching - only if elements exist
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Only set up navigation if elements exist (for pages that have them)
    if (navRecorder && navSurvey && recorderView && surveyView) {
        navRecorder.onclick = () => {
    ...
    // Handle survey submission if form exists
...
const createWaveSurfer = () => {
  ...
  record = wavesurfer.registerPlugin(
    ...
    const updateGlobalControls = () => {
        ...
        if (selectedSlot && !selectedSlot.classList.contains('empty')) {
                ...
                if (selectedSlot.dataset.metadata) {
                    ...
                    fetch(recordedUrl)
                        ...
                        .then(async audioBlob => {
                            ...
                            if (selectedSlot.dataset.description) {
                                metadata.description = selectedSlot.dataset.description
                            }
            ...
            document.querySelector('#global-tag').onclick = () => {
                ...
                const saveBtn = document.querySelector('#save-description')
                const closeBtn = document.querySelector('#close-description')

                // Show modal
                descriptionModal.style.display = 'block'

                // Load existing description
                const existingDescription = selectedSlot.dataset.description || ''
                ...
                // Function to update description badge
                function updateDescriptionBadge() {
                    let badge = selectedSlot.querySelector('.description-badge')
                    if (selectedSlot.dataset.description) {
                        if (!badge) {
    ...
    saveAllButton.onclick = async () => {
        ...
        // Collect all recordings data with actual files
        for (let i = 0; i < slots.length; i++) {
            ...
            if (!slot.classList.contains('empty')) {
                ...
                const recordingData = {
                    ...
                    metadata: slot.dataset.metadata ? JSON.parse(slot.dataset.metadata) : {},
                    description: slot.dataset.description || '',
                    hasPhoto: !!slot.dataset.photoUrl,
                    slot: slot
                }
                
                // Add description to metadata
                if (recordingData.description) {
                    recordingData.metadata.description = recordingData.description
                }
                
                recordingsData.push(recordingData)
            }
        }
        ...
        console.log('Recordings data to save:', recordingsData)
        console.log('Number of recordings:', recordingsData.length)

        try {
            // Show saving indicator
            saveAllButton.disabled = true
            saveAllButton.innerHTML = '<img src="icons/refresh-cw.svg" alt="Saving..." style="animation: spin 1s linear infinite;">'
            
            // Save to backend server using FormData for files
            ...
            for (const recording of recordingsData) {
                ...
                const baseKey = `recording_${recording.slotNumber}`
                
                console.log(`Processing slot ${recording.slotNumber}:`, {
                    hasAudio: !!slot.dataset.recordedUrl,
                    hasPhoto: !!slot.dataset.photoUrl,
                    description: recording.description
                })
                
                // Add metadata
                formData.append(`${baseKey}_metadata`, JSON.stringify(recording.metadata))
                formData.append(`${baseKey}_description`, recording.description || '')
                
                // Add audio file
                if (slot.dataset.recordedUrl) {
                    try {
                        console.log(`Adding audio for slot ${recording.slotNumber}`)
                        const audioResponse = await fetch(slot.dataset.recordedUrl)
                        const audioBlob = await audioResponse.blob()
                        formData.append(`${baseKey}_audio`, audioBlob, `${recording.filename}`)
                        console.log(`Audio added for slot ${recording.slotNumber}, size:`, audioBlob.size)
                    } catch (error) {
                        console.warn(`Could not add audio for slot ${recording.slotNumber}:`, error)
                    }
                }
                ...
                if (slot.dataset.photoUrl) {
                    try {
                        console.log(`Adding photo for slot ${recording.slotNumber}`)
                        const photoResponse = await fetch(slot.dataset.photoUrl)
                        const photoBlob = await photoResponse.blob()
                        formData.append(`${baseKey}_photo`, photoBlob, `photo-${recording.slotNumber}.jpg`)
                        console.log(`Photo added for slot ${recording.slotNumber}, size:`, photoBlob.size)
                    } catch (error) {
                        console.warn(`Could not add photo for slot ${recording.slotNumber}:`, error)
                    }
                }
            }
            ...
            const response = await fetch('/api/save-recordings', {
                method: 'POST',
                body: formData
            })
            ...
            if (response.ok) {
                ...
                alert(`Successfully saved ${recordingsData.length} recordings to the server!`)
                
                // Also create and download zip file for local backup
                await createAndDownloadZip(recordingsData)
            } else {
                throw new Error(`Server error: ${response.status}`)
            }
        } catch (error) {
            console.error('Error saving to server:', error)
            alert('Failed to save to server. Creating local backup instead.')
            
            // Fallback to local zip download
            await createAndDownloadZip(recordingsData)
        } finally {
            // Reset button state
            saveAllButton.disabled = false
            saveAllButton.innerHTML = '<img src="icons/save.svg" alt="Save All">'
        }
    }
    
    // Enable save all button if there are any recordings
    saveAllButton.disabled = false

    // Function to create and download zip file
    async function createAndDownloadZip(recordingsData) {
        const zip = new JSZip()
        
        for (const recording of recordingsData) {
            const slotFolder = zip.folder(`slot-${recording.slotNumber}`)
            
            // Add metadata
            slotFolder.file('metadata.json', JSON.stringify(recording.metadata, null, 2))
            
            // Add audio file (if we can access it)
            try {
                const slot = document.querySelector(`.recording-slot:nth-child(${recording.slotNumber})`)
                if (slot && slot.dataset.recordedUrl) {
                    const audioResponse = await fetch(slot.dataset.recordedUrl)
                    const audioBlob = await audioResponse.blob()
                    slotFolder.file(recording.filename, audioBlob)
                }
                
                // Add photo if exists
                if (slot && slot.dataset.photoUrl) {
                    const photoResponse = await fetch(slot.dataset.photoUrl)
                    const photoBlob = await photoResponse.blob()
                    slotFolder.file('photo.jpg', photoBlob)
                }
            } catch (error) {
                console.warn(`Could not add files for slot ${recording.slotNumber}:`, error)
            }
        }
  ...
  record.on('record-progress', (time) => {
    ...
    // Initialize save all button state
    updateSaveAllButton()
}
...
// Function to check if there are any recordings and update save all button
function updateSaveAllButton() {
    ...
    saveAllButton.disabled = !hasRecordings
}
